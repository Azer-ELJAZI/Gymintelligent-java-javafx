...
public class CoachInterface {
    ...
    public static void display() {
        ...
        StackPane AdminLayout = new StackPane();
           ...
           Label titleLabel = new Label("                                                        ***Welcom Coach***");
...
programe.setPromptText("Enterle programe  de votre client ");
// Add a Text to display the advice
 GestionPrograme gp=new GestionPrograme();
// Add Button to trigger afficherConseilByPhone
Button  ajouterProgrameBtn = new Button("ajouter programe ");
 ajouterProgrameBtn.setPrefSize(400, 20);
        ajouterProgrameBtn.setBackground(new Background(new BackgroundFill(Color.DIMGREY, new CornerRadii(5), Insets.EMPTY)));
ajouterProgrameBtn.setOnAction(e -> {
    ...
    if (!phoneText.isEmpty()) {
        try {
          ...
          e1.chercherCoachByMatricule(a).enregistrerProgramme2(atricule, ProgrameText);
            gp.ajouterPrograme(phoneText90,ProgrameText);
...
Button afficherSalaireBtn = new Button("Afficher l'employee ");
 afficherSalaireBtn.setPrefSize(400, 20);
        afficherSalaireBtn.setBackground(new Background(new BackgroundFill(Color.BURLYWOOD, new CornerRadii(5), Insets.EMPTY)));
...
Button afficherfichierBtn = new Button("Afficher fichier de mon client ");
 afficherfichierBtn.setPrefSize(400, 20);
        afficherfichierBtn.setBackground(new Background(new BackgroundFill(Color.ANTIQUEWHITE, new CornerRadii(5), Insets.EMPTY)));
afficherfichierBtn.setOnAction(e -> {
    String phoneText = matriculeToDisplaySalaire.getText();
    if (!phoneText.isEmpty()) {
        try {
            int atricule = Integer.parseInt(phoneText);
          e1.chercherCoachByMatricule(atricule).afficherFichierPourEmployer();
            conseilText.setText( e1.chercherCoachByMatricule(atricule).afficherFichierPourEmployer());
        } catch (NumberFormatException ex) {
            System.out.println("Veuillez entrer un numéro de téléphone valide.");
        }
    } else {
...
vbox8.getChildren().addAll(
       ...
       r.setTranslateX(400);
        ...
        Image image14 = new Image(getSystemResourceAsStream("image/sport3.jpg"));
